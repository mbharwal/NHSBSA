...
public class StepDefs implements En {
    ...
    public StepDefs() {
        ...
        Given("I am a person from {string}", (String string) -> {
            startPage.openURL();
            startPage.start();
            locationPage.chooseLocation(string);
        });


        When("enter date {int}", (Integer date) -> {
            dobPage.date(date);
        });

        When("enter month {int}", (Integer month) -> {
            dobPage.month(month);
        });

        When("enter year {int}", (Integer year) -> {
            dobPage.year(year);
